class InvalidActivity(Exception):
    def __init__(self, name):
        super().__init__(
            f"{name} is not a valid activity name, as it does not match [a-z][a-z_0-9]*!"
        )
...
class InvalidRandomnessParameters(Exception):
    def __init__(self, name, params):
        super().__init__(
            f"{name} cannot be called with parameters {params}. Check numpy documentation."
        )
...
class UnknownDeclareConstraint(Exception):
    def __init__(self, name):
        ...
        msg = [
            f"Unknown DECLARE constraint: {name}.\n Available ones (not case-sensitive):"
        ]
