...
def to_logic_facts(event_log_df, output_file):
    ...
    for (case_id, partition), _ in event_log_df.groupby(
        ["case:concept:name", "case:concept:partition"]
    ):
...
def write_to(log, path):
    dict2fun = {"txt": to_strings, "lp": to_logic_facts, "xes": to_xes, "csv": to_csv}
