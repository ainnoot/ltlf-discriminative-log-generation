...
def event_log_dataframe_from_dict_of_traces(dict_of_traces):
    event_log = pandas.DataFrame(columns=['case:concept:name', 'case:concept:partition', 'concept:name', 'time:timestamp'])
    case_id = 1

    for label, traces in dict_of_traces.items():
        for trace in traces:
            event_log = pandas.concat([event_log, trace_to_dataframe(trace, case_id, label)])
            case_id += 1
